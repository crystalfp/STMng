...
onMounted(() => {
    ...
    receiveInWindow((data) => {
        ...
        for(const entry of info.allNodes) {
            ...
            listRight.push({name: label, id, type: entry.type, isNew: true, selected: false});
        }
        ...
        listRight.sort((a, b) => a.name.localeCompare(b.name));
...
<v-dialog v-model="showReorder">
  <v-card title="Reorder project nodes" class="mx-auto no-select" elevation="16" width="700">
    <v-card-text>
      <v-row class="pb-0">
        <v-col cols="6">
          <v-label class="column-title">In use</v-label>
        </v-col>
        <v-col cols="6">
          <v-label class="column-title">Available</v-label>
        </v-col>
      </v-row>
