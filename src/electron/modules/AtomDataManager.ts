...
import type {AtomInfo} from "./AtomData";
...
const loadAtomData = (filePath: string): void => {
	...
	console.log("loadAtomData", data); // TBD
};
...
export const selectAtomDataFile = (useDefault: boolean): void => {
	...
	// const currentUse = store.getBoolean("UseDefaultAtomData", true);
	const currentUse = getAtomDataDefault();
	if(currentUse === useDefault) return;

	// store.setBoolean("UseDefaultAtomData", useDefault);
...
export const setupChannelAtomData = (): void => {
	...
	ipcMain.handle("ATOM-DATA:GET", () => {
		...
		return {
			...
			data: JSON.stringify(atomData)
		};
	});

	ipcMain.on("ATOM-DATA:SET", (_event, params: {useDefault: boolean; data: string}) => {

		// store.setBoolean("UseDefaultAtomData", params.useDefault);
